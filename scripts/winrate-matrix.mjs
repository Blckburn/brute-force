...
/**
 * Пары архетипов — не весь риск. Трейты одной школы умножают друг друга,
 * и связка может выйти за коридор там, где каждый её участник по
 * отдельности в него укладывается. Проверяются связки, у которых
 * множители складываются в цепочку.
 *
 * MAG опасна прежде всего: `plaguebearer` вешает яд на каждом попадании,
 * `hexblade` — хекс, `amplifier` усиливает и то и другое.
 */
const COMBOS = [
...
if (AS_JSON) {
  ...
  console.log('Ожидание: трейты усиливают, поэтому винрейт выше 50%. Насколько выше —');
  console.log('и есть вопрос: связка, стабильно берущая больше 80%, отменяет выбор.');
  console.log('─'.repeat(64));
  ...
  const GDD_FIXED = ['warlord', 'cursed', 'fortress', 'thorns', 'phantom', 'hexblade'];
