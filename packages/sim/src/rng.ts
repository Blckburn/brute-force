...
export function rngFromState(initial: RngState): Rng {
  ...
  return {
    ...
    /**
     * Бросок с вероятностью `p`.
     *
     * **Бросок делается ВСЕГДА, даже при нулевой и единичной вероятности.**
     * Ранний выход выглядел бы экономией, но делает поток генератора
     * зависимым от ЗНАЧЕНИЯ коэффициента: два билда, отличающиеся только
     * тем, что у одного шанс равен нулю, расходились бы с первого же
     * такого броска и давали разные бои на одном сиде.
     *
     * Поймано калибровкой: `slippery` с множителем крита 0.05 давал 60.6%
     * побед, а с множителем 0 — 64.4%. Разница в полпроцента шанса не
     * может стоить четырёх пунктов; это была не более сильная механика,
     * а другая выборка. Матрица, которой правят числа, обязана мерить
     * изменение числа, а не смещение потока.
     */
    chance(p: number): boolean {
      const roll = next();
      if (p <= 0) return false;
