...
describe('сам генератор', () => {
  ...
  it('chance на границах не бросает вовсе', () => {
    const rng = rngFromSeed('edges');
    for (let i = 0; i < 100; i++) {
      expect(rng.chance(0)).toBe(false);
      expect(rng.chance(1)).toBe(true);
    }
    // Состояние не сдвинулось: вырожденные вероятности не тратят броски.
    const a = rngFromSeed('edges');
    expect(rng.next()).toBe(a.next());
  });
});
