...
describe('внутри школы нет обязательного пика', () => {
   ...
   *
   * Они исключены из разброса не для того, чтобы тест позеленел, а потому
   * что двигать эти числа агент не вправе: правка документа — решение
   * человека. Двое из шести сейчас далеко за рамками — `cursed` около 15%,
   * `thorns` около 87%, — и это ЗАФИКСИРОВАННАЯ НАХОДКА, вынесенная
   * человеку, а не молча спрятанный провал.
   *
   * Список задан явно и сверяется целиком: без этого исключение однажды
   * разрослось бы до «всё, что не сходится».
   */
  const GDD_FIXED = ['warlord', 'cursed', 'fortress', 'thorns', 'phantom', 'hexblade'] as const;

  it('исключены из калибровки ровно шесть аудитных трейтов, не больше', () => {
    expect([...GDD_FIXED].sort()).toEqual([
      'cursed',
      'fortress',
      'hexblade',
      'phantom',
