 ...
 * и чтобы кэшировать превью (GDD §5.4).
 *
 * Это НЕ источник данных о снаряжении: состав экипировки сервер читает
 * из БД (инвариант 1, GDD §2.2 шаг 1). Хеш — только для сверки и кэша.
 */
...
export const simulatePreviewInputSchema = z.object({
  ...
  loadoutHash: loadoutHashSchema,
  /**
   * Сколько прогонов Монте-Карло. GDD §5.4 фиксирует 300; параметр
...
/**
 * Формат боевого лога определяется в M1 вместе с движком (GDD §2.2).
